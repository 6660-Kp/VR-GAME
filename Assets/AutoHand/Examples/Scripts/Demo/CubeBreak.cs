    ...
    public class CubeBreak : MonoBehaviour{
        ...
        public void Break() {
            for(int i = 0; i < 8; i++) {
                ...
                smallerCopy.GetComponent<Rigidbody>().AddRelativeForce(transform.rotation*(offsets[i]*force + Vector3.one*(Random.value/3f)), ForceMode.Impulse);
                smallerCopy.GetComponent<Rigidbody>().AddRelativeTorque(transform.rotation*(offsets[i]*force), ForceMode.Impulse);
